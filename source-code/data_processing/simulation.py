...
def simulate_production(
    ...
    if 'Wind_Onshore' in capacity_dict:
        ...
        if target_capacity > 0:
            ...
            possible_cols = ['Wind Onshore [MWh]', 'Wind_Onshore [MWh]', 'Wind Onshore', 'Wind_Offshore', 'Onshore [MWh]']
            wind_on_col = None
...
def kobi(
    ...
    for year in years:
        ...
        try:
            ...
            # --- E-Mobilität (V2G) Integration ---
            current_df = df_bal
            
            # Versuche E-Mobility Parameter zu laden
            emob_conf = sm.scenario_data.get("consumption_strategies", {}).get("emobility_v2g", {})
            if not emob_conf and "emobility" in sm.scenario_data: # Fallback Top-Level
                 emob_conf = sm.scenario_data["emobility"]

            if emob_conf.get("active", False):
                try:
                    # Versuche Profil zu laden
                    df_ev_profile = None
                    # Prüfen ob im DataManager registriert (z.B. als ID oder Name)
                    # Einfacher Fallback: Direkter Load wenn nicht gefunden
                    try:
                         df_ev_profile = dm.get("emobility_profile_S2030") # Versuche Standard-Namen
                    except:
                         pass
                    ...
                    if df_ev_profile is None:
                        ...
                        p = Path(__file__).parent.parent.parent / "raw-data" / "emobility_profile_S2030.csv"
                        if p.exists():
                             df_ev_profile = pd.read_csv(p)
                    
                    if df_ev_profile is not None:
                        # Parameter laden
                        # Struktur in YAML kann variieren, wir supporten:
                        # emobility_v2g:
                        #   active: true
                        #   2030: { n_cars: 5000000 }
                        #   battery_capacity_kwh: 50
                        
                        yr_conf = emob_conf.get(year, {})
                        if not yr_conf and year in emob_conf: # Falls Key int ist
                             yr_conf = emob_conf[year]
                        
                        n_cars = yr_conf.get("n_cars", 0)
                        # Wenn year-config leer, schaue ob n_cars global definiert (unwahrscheinlich aber möglich)
                        if n_cars == 0: n_cars = emob_conf.get("n_cars", 0)

                        batt_cap = emob_conf.get("battery_capacity_kwh", 50.0)
                        chg_pow = emob_conf.get("charging_power_kw", 11.0)
                        
                        # Erweiterte Parameter laden
                        soc_conf = emob_conf.get("soc_limits", {})
                        grid_conf = emob_conf.get("grid_thresholds_mw", {})

                        if n_cars > 0:
                            print(f"Simuliere E-Mobilität: {n_cars:,} Fahrzeuge im Jahr {year}")
                            current_df = simulate_emobility_fleet(
                                df_balance=current_df,
                                df_ev_profile=df_ev_profile,
                                n_cars=n_cars,
                                battery_capacity_kwh=batt_cap,
                                charging_power_kw=chg_pow,
                                soc_config=soc_conf,
                                grid_config=grid_conf
                            )
                except Exception as ex:
                    print(f"Fehler bei V2G Simulation: {ex}")
