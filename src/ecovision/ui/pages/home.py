...
def home_page() -> None:
    ...
    is_loaded = st.session_state.dm is not None
    ...
    with col1:
        if st.button("Main Simulation", width='stretch', icon=":material/table_chart_view:", key="home_nav_simulation"):
            st.switch_page(st.session_state.pages["simulation"])
    
    with col2:
        if st.button("Szenario Konfiguration", width='stretch', icon=":material/tune:", key="home_nav_scenario"):
            st.switch_page(st.session_state.pages["scenario"])
    
    with col3:
        if st.button("Daten Analyse", width='stretch', icon=":material/area_chart:", key="home_nav_analysis"):
